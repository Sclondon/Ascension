...
@export var min_distance_between_platforms: float = 1.0  # Minimum distance between platforms
...
func _on_new_section_added(height: float):
	...
	var attempted_spawn = 0  # Counter for how many times we try to spawn a platform

	while attempted_spawn < platforms_to_spawn:
		if randf() < spawn_probability:  # Chance to spawn each platform
			...
			if _is_position_valid(position):  # Check if the position is valid
				...
				attempted_spawn += 1  # Successfully spawned a platform
...
func _get_random_angle() -> float:
	...
	return angle_step * (randi() % number_of_sides) + angle_step * (randf() * 0.25)  # Randomly choose one of the sides and add a little randomness
...
func _is_position_valid(new_position: Vector3) -> bool:
	for platform in platforms:
		...
		if platform.global_transform.origin.distance_to(new_position) < min_distance_between_platforms:
...
func remove_old_platforms():
	if platforms.size() > 0:
		var old_platform = platforms.pop_front()
		old_platform.queue_free()

	print("Old platform removed.")
